...
public class RhythmGenerator {
    ...
    public int[][] abcToIntRhythmDatabase(String dataPath, String timeSignature) throws Exception{
		...
		for(int i=0; i<notations.length; i++) {
			if(getTimeSignature(notations[i]).equals(timeSignature))// filtering the dataset based on the given time signature
				result.add(abcToIntRhythm(notations[i], timeSignature));// adding the pattern sequences to the list
		}
    ...
    public int[] abcToIntRhythm(String abcNotation, String timeSignature) throws Exception {
    	    ...
    	    if(timeSignature.equals("4/4"))
    	    	return patternReader(intLengths, patterns_4_4); 
    	    return null;// add for more pattern maps
    ...
    //sad funkcija koja dodaje na svaki bar i odvaja barovi sa |
    // funkcija koja sve spaja u jedan abc string
